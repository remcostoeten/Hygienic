...
import { TConsolidationResult, TImportInfo } from './types';
...
export class UIImportConsolidator {
  ...
  async checkGitStatus(force: boolean = false): Promise<boolean> {
    try {
      ...
      if (stdout.trim() && !force) {
        this.log('Error: Git has uncommitted changes. Use --force to proceed anyway.', Colors.red);
        return false;
      }
      return true;
    } catch {
      ...
      for (const projectDir of projectDirs) {
        ...
        if (stat.isDirectory()) {
          ...
          for (const timestampDir of timestampDirs) {
            try {
              const dirDate = new Date(timestampDir.replace(/_/g, ':').replace('-', 'T') + '.000Z');
  ...
  private async consolidateImports(
    ...
    try {
      ...
      const sortedNames = sortImports
        ? Array.from(allUINames).sort()
        : Array.from(allUINames);
      ...
      const originalImports = uiImports.map(imp =>
        `import { ${Array.from(imp.names).sort().join(', ')} } from '${imp.module}';`
      );
    ...
    for (const path of paths) {
      try {
        ...
        if (stat.isFile()) {
          ...
          const foundFiles = await this.findTSXFiles(path, includePatterns, excludePatterns);
      ...
      try {
        ...
        if (result.changed) {
          if (dryRun) {
            ...
            this.log(`[CHANGE] Consolidated imports in ${filePath} → backup created`, Colors.green);
