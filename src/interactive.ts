...
export async function interactiveConfig(config: Config) {
  ...
  for (const [key, currentValue] of Object.entries(config.data)) {
    if (Array.isArray(currentValue)) {
      ...
      if (newValue.trim()) {
        await config.set(key as any, newValue.split(',').map(s => s.trim()));
      }
    } else if (typeof currentValue === 'boolean') {
      const newValue = await question(`${key} [${currentValue}] (true/false): `);
      if (newValue.trim().toLowerCase() === 'true' || newValue.trim().toLowerCase() === 'false') {
...
export function interactiveHistory(history: History) {
  ...
  recent.forEach((run, i) => {
    console.log(`${(i + 1).toString().padStart(2)}. [${run.timestamp}] ${run.status} - ${run.filesChanged}/${run.filesProcessed} files`);
    console.log(`    Paths: ${run.paths.slice(0, 3).join(', ')}${run.paths.length > 3 ? '...' : ''}`);
