...
import { createTestConfig, cleanup, createTestFile } from './utils';
...
export function Dashboard() {
  return (
    <div>
      ...
      expect(homeResult?.consolidatedImport).toBe("import { Button, Card, Input } from '@/shared/components/ui';");

      expect(loginResult?.changed).toBe(true);
      expect(loginResult?.consolidatedImport).toBe("import { Button, Dialog, Input } from '@/shared/components/ui';");

      expect(dashboardResult?.changed).toBe(true);
      expect(dashboardResult?.consolidatedImport).toBe("import { Button, Card } from '@/shared/components/ui';");

      const modifiedHomePage = await fs.readFile(join(tempDir, 'src/pages/HomePage.tsx'), 'utf-8');
      expect(modifiedHomePage).toContain("import { Button, Card, Input } from '@/shared/components/ui';");
...
export function ContactForm() {
  ...
  return (
    <form>
      ...
      expect(results[0].consolidatedImport).toBe("import { Button, Input, Label, Select, Textarea } from '@/shared/components/ui';");

      const modifiedContent = await fs.readFile(join(tempDir, 'src/components/ContactForm.tsx'), 'utf-8');
      expect(modifiedContent).toContain("import { Button, Input, Label, Select, Textarea } from '@/shared/components/ui';");
      expect(modifiedContent).toContain("import React, { useState } from 'react';");
      expect(modifiedContent).toContain("import { zodResolver } from '@hookform/resolvers/zod';");
...
export function NoUIComponent() {
  ...
  return <div onClick={() => setCount(count + 1)}>{count}</div>;
      ...
      await createTestFile(tempDir, 'src/components/AlreadyGood.tsx', alreadyConsolidatedContent);
      ...
      const alreadyGoodContent = await fs.readFile(join(tempDir, 'src/components/AlreadyGood.tsx'), 'utf-8');
...
export function ComponentWithComments() {
  return (
    <div>
      ...
      const modifiedContent = await fs.readFile(join(tempDir, 'src/components/Commented.tsx'), 'utf-8');

      expect(modifiedContent).toContain('// This is a header comment');
      expect(modifiedContent).toContain('/**');
      expect(modifiedContent).toContain('*/');
      expect(modifiedContent).toContain('{/* Button comment */}');
      expect(modifiedContent).toContain("import { Button, Input } from '@/shared/components/ui';");
