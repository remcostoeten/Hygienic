...
import { createTestConfig, cleanup, createTestFile, createMockTSXContent, createBarrelFileContent } from './utils';
...
describe('UIImportConsolidator', () => {
  ...
  describe('file operations', () => {
    ...
    test('should create backups when applying changes', async () => {
      ...
      const backupExists = await fs.access(results[0].backupPath!).then(() => true).catch(() => false);
  ...
  describe('caching', () => {
    ...
    test('should skip cached files', async () => {
      ...
      const results = await consolidator.processFiles([tsxFile], true, false, undefined, undefined, true);
    ...
    test('should process files when cache is disabled', async () => {
      ...
      const results = await consolidator.processFiles([tsxFile], true, false, undefined, undefined, false);
